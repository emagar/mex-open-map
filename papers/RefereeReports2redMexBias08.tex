...
\end{itemize}
\item {\bfseries\sffamily WORK\_IN\_PROGRESS} Table 3 uses OLS regression for deriving swing ratios: "We derive swing ratios by regressing a party's seat shares in simulated elections on the party's simulated vote shares." Does this regression account for the uncertainity/credibility of simulated shares?
\label{sec:orgheadline30}
\begin{itemize}
\item EM suggested action: explain that it does.
\item Linzer approximates weird density of district party perfromance correlations with mix. Then a large number of national elections are simulated with that density, random noise around observed patterns. OLS gives central tendency, SEs are measure of uncertainty.
\item Check what Linzer has to say about this, quote him.
\end{itemize}
\end{enumerate}
\section{ToDo list}
