...
- EM suggested action: concede, but not too easy given downplay of the Mexican case.
- Most interesting case that we can mention is Britain, mention to MM's student work. Canada, India should be interesting too.
* Reviewer #4
...
** WORK_IN_PROGRESS Table 3 uses OLS regression for deriving swing ratios: "We derive swing ratios by regressing a party's seat shares in simulated elections on the party's simulated vote shares." Does this regression account for the uncertainity/credibility of simulated shares?
- EM suggested action: explain that it does.
- Linzer approximates weird density of district party perfromance correlations with mix. Then a large number of national elections are simulated with that density, random noise around observed patterns. OLS gives central tendency, SEs are measure of uncertainty.
- Check what Linzer has to say about this, quote him.
* ToDo list
