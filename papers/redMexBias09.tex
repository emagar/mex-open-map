...
\end{tabular}
\caption{Relative partisan bias and its additive components. Entries report the median of the posterior sample of parameters estimated with the single-election models. Numbers in parentheses are the share of the posterior sample with sign opposite to the reported point estimate's. The right columns report bias estimates using election data re-arranged according to new district boundaries (i.e., a hypothetical 2003 election with the 2006 map and a hypothetical 2015 election with the map not implemented).}\label{T:GKBbreakdown}
\end{table}

Table \ref{T:GKBbreakdown} reports the estimated total and additive components of partisan bias. Bias estimates for the PAN, the PRD, and one minor party (the green) relative to PRI's for the elections as they were conducted in the implemented maps are presented in the first three columns. Numbers in parentheses report the share of the posterior sample with a sign opposite to that reported in the table, serving as a test of the estimate's statistical significance (the probability that the estimate's sign is wrong). 
...
We offer here a step-by-step description of the procedure's implementation for analysis of recent Mexican elections. Code to replicate the analysis is available *here*.  
