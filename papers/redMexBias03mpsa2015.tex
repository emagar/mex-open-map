...
% \begin{abstract}
% \noindent  Mexico´s purportedly electoral management body (EMB)---INE, formerly IFE---has carried out automated redistricting processes since the mid 1990s. In such process, political parties participate by formulating counterproposals to a technical committee once a machine generated scenario is generated. From our prespective, this has become a natural laboratory to analyze different sources of party bias in the plurality tier of the mutiparty mixed system to elect the lower chamber of Congress. Based on Monte Carlo simulations of recent elections, we develop a novel procedure to measure three potential sources of party bias separately: substantial malapportionment, gerrymandering, and turnout. Our results show that, relative to the right-of-center PAN, the former hegemonic PRI, but especially the left-of-center PRD have enjoyed advantage in the votes to seats conversion. Our analysis also reveals how systematic and often substantive turnout-based bias in favor of the PRI has been countered by district boundaries substantively helping one or both other major parties. Despite nominally neutral district drawing, gerrymandering remains a key ingredient of the mix for party bias in representation.
% \end{abstract}

\begin{abstract}
\noindent Mexico´s federal electoral authority has carried out automated redistricting since the mid 1990s. We inspect two recent machine-generated maps for evidence of party bias in the translation of votes to seats for the lower chamber of Congress. Based on Monte Carlo simulations of recent elections, we develop a new procedure to measure three potential sources of party bias in a multiparty system separately: malapportionment, gerrymandering, and turnout. Analysis reveals how, relative to the right-of-center PAN, the former hegemonic PRI, but especially the left-of-center PRD have enjoyed advantageous seats:votes ratios. We also show that systematic and often large turnout-based bias in favor of PRI has been countered by district boundaries substantively helping one or both other major parties. Despite nominally neutral district drawing, gerrymandering remains a key ingredient of party bias in representation.
\end{abstract}

%Alejandro: The first sentence of the abstract "The 1997 electoral reform rid Mexico of the most flagrant distortions on representation" seemed a bit out of context for the broader audience and disconnected from the rest of the abstract, so I took it out and replaced it with Mexico's automated redistricting context. 
%% eric [6/19/15] Made Alejandro's abstract more compact (kept his commented, above)
...
% Strategy paper: touch issue of weights, units and components of cost function in general---rabbit burgers from how to lie with statistics, one unit of rabbit and one unit of cow... Eric: that humans beat the machine is proof that the algorithm stops at local minima quite easily. 
...
% It also has a population largely matching the PRI's traditional bases of electoral support: rural population, less educated, and in general the less-well-to-do \citep{moreno.votMex.2003,magar.1994,amesMex.1970}. Gudgin, Graham, and Peter J. Taylor. 1979. Seats, Votes and the
...
In this paper we use the model developed by \citet{king.1990elRespBiasMultiparty}, generalizing the fit of a vote--seat curve, to calculate our estimators. Furthermore, we replicate the procedure formulated by \citet{grofman.etalBiasMalapp.1997} to decompose estimates of party bias into three additive components. Using this general method (devised for a two-party system), in combination with King's model, we surmounted the obstacle of multiparty competition in Mexico. Given the small number of elections celebrated since the system became democratic, and following recent work by \citet{linzerSeatVoteElasticity2012}, we used Monte Carlo draws to produce a large number of simulated national election outcomes consistent with the observed district-level distribution of partisan support. 

The remainder of the article is structured as follows. The next section discusses partisan bias, the three sources behind it (the distributive or gerrymandering-, the turnout-, and the malapportionment-based), and how operationalizing parties' national vote shares in slightly different ways clear the way to measure them empirically. The third section investigates malapportionment in Mexican elections, showing that it is substantial due to belated redistricting---when carried five or more years after the census, as has been done, population information is outdated. The fourth section derives expectations of bias from the partisan model of electoral regulation. District boundaries in Mexico are not drawn by politicians but by experts under the aegis of a collegiate electoral regulator where all three major parties share power \citep{estevez.magar.rosas.2008}. This removes major suspicions of flagrant manipulation, but the question of partisan gerrymandering subsits. The fifth section describes estimation methods and the sixth section elaborates on our data. The seventh section reports our party bias results, both in terms of raw party bias and in terms of its three, additive components. An eighth section concludes. 

% Alejandro: Not clear how the fourth section interacts with the second section. Maybe it should read? Here is what we know about these three things (section 2), then we present a methodology (sections 5 and 6), then we investigate malapportionment (sections 3, 4,7 and 8). Micah will read again and restructure if needed.
...
\end{center}
\end{figure}
%Alejandro: This plot should go after the first paragraph, not before. For some reason, the colors are no longer available in the graph.
