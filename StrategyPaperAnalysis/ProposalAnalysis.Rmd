...
ys.proposed %<>% mutate(clm50 = ifelse(clm.residuals >= 0.5, 1, 0))
ys.proposed %<>% mutate(clm90 = ifelse(clm.residuals >= 0.9, 1, 0))
...
ys.proposed %>% filter(partytype == "major" | partytype == "minor") %>% ggplot(aes(StratPos, clmm.residuals)) + geom_jitter()
...
ys.proposed %>% filter(clmm.residuals > 0.5) %>% ggplot(aes(StratPos, clmm.residuals)) + geom_jitter() + facet_wrap(~actor)
...
ys.proposed %<>% mutate(clmm50 = ifelse(clmm.residuals >= 0.5, 1, 0))
ys.proposed %<>% mutate(clmm90 = ifelse(clmm.residuals >= 0.9, 1, 0))

#Residual charts for cutoff categorizations.

ys.proposed %<>% mutate(clmm50.resid = didPropose.num-clmm50)
ys.proposed %>% filter(partytype == "major" | partytype == "minor") %>% ggplot(aes(StratPos, clmm50.resid)) + geom_jitter()
