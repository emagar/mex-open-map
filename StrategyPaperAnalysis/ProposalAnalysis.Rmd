...
#CLM model first
clm.mod <- ys.proposed %>% clm(as.ordered(didPropose)~as.ordered(StratPos), data=.)
summary(clm.mod)
confint(clm.mod)

#example of ordinal model only, seems to have numerical problems

model.results2 <- ys.proposed %>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+as.factor(year)+(1|actor),., Hess = TRUE)
summary(model.results2)
confint(model.results2)
qqplot(model.results2)

anova(model.results2, clm.mod)
...
#clmm2
clmm2.mod <- clmm2(location = as.ordered(didPropose)~ as.ordered(StratPos) + as.factor(year) + (1|actor), data = ys.proposed, Hess = TRUE)
summary(clmm2.mod)
exp(coef(clmm2.mod)[2])
exp(coef(clmm2.mod)[3]) 
exp(coef(clmm2.mod)[4])
exp(coef(clmm2.mod)[5])
exp(coef(clmm2.mod)[6])

clmm2.mod
#Confidence Intervals
confint(clmm2.mod, level=0.95, type=c("profile","Wald"))

CI <- profile(clmm2.mod)
print(CI)
confint(CI)
plot(CI)
