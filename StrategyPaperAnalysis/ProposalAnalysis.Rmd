...
```{r}
#Bringing in the Vote Share data
Vraw.2015 <- read.csv("dipfed-seccion-vraw-2015.csv")

#Omit missing data
Vraw.2015 <- na.omit(Vraw.2015)

#Sum votes across seccion; aggregated at the state level
Vraw.2015 %<>% aggregate(., by = list(Vraw.2015$edon), FUN = sum) %>%
  select(-Group.1)%>%
#Generate a year variable (anticipating multiple years of data)
  mutate(year = 2015)%>%
#Recalculate the dummies for coalitions
  mutate(dpanc = ifelse(dpanc > 0, 1, 0)) %>%
  mutate(dpric = ifelse(dpric > 0, 1, 0)) %>%
  mutate(dprdc = ifelse(dprdc > 0, 1, 0))

#Index the states
Vraw.2015 <- tibble::rowid_to_column(Vraw.2015, "edon_r")
...
Vraw.2015 %<>% mutate(VoteShare = (votes/efec)*100) 
```
...
#Rowbind into 1) 2013 Information 2) 2017 information

#aggregate func. Take average of votes in each seccion for each party grouping. group_by(edon, seccion)

#Calc the vote share

#Join 1) Vhat2012 2) vhat 2015
