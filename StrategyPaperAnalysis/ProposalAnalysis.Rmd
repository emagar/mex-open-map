...
ys.proposed <- ys.proposed %>% mutate(StratPos.1 = ifelse(didControl & singlecontrol, 1, 0)) %>%
  ...
  mutate(StratPos.5 = ifelse(!didControl & percentsingle < 0.5 & numparty >= 3, 1, 0))

tabfit.1 <- ys.proposed %>% xtabs(~StratPos.1+StratPos.2+StratPos.3+StratPos.4 + StratPos.5, data=.)
print(tabfit.1)
ftable(tabfit.1)
