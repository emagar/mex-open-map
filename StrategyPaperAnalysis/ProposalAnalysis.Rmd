...
#check pattern of proposal entry
 ys.proposed %>% xtabs(~StratPos+didPropose,.)
 
 #example of ordinal model only, seems to have numerical problems
  ys.proposed %>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+(1|actor),.)
...
prop.H1.df <- prop.H1.df %>% mutate(id = row_number())
train <- prop.H1.df %>% sample_frac(.70)
test <- anti_join(prop.H1.df, train, by="id")
test <- as.data.frame(test)

#1. Logistic
Mod.1 <- train %>% glm(PROPOSED ~ as.factor(StratPos), data=., family = binomial)
Mod.1.pred <- predict(Mod.1, test)
...
t <- table(Mod.1.pred, test$PROPOSED)
t
```
