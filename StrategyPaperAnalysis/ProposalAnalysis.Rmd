...
ys.proposed <- ys.proposed %>% mutate(StratPos.1 = ifelse(didControl & singlecontrol, 1, 0)) %>%
  mutate(StratPos.2 = ifelse(didControl & !singlecontrol, 1, 0)) %>%
  mutate(StratPos.3 = ifelse(!didControl & percentsingle>=0.5,1, 0)) %>%
  mutate(StratPos.4 = ifelse(!didControl & percentsingle < 0.5 & numparty >= 3, 1, 0))
...
tab.3 <- prop.H1.df %>% xtabs(~StratPos.1+StratPos.2+StratPos.3+StratPos.4+StratPos.5, data=.)
print(tab.3)
ftable(tab.3)
#Visualizations by state and year

by.year.17 <- prop.H1.df %>% filter(year == 2017)
hist(by.year.17$stage)

tabfit.2 <- prop.H1.df %>% xtabs(~stage+PROPOSED+year+singlecontrol, data=.)
print(tabfit.2)
