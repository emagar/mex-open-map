...
ys.proposed %>% filter(actortype == "major" | actortype == "minor") %>% ggplot(aes(StratPos, clm.residuals, color=actor)) + geom_jitter() + facet_wrap(~actortype)
ys.proposed %>% filter(actortype == "major" | actortype == "minor") %>% ggplot(aes(StratPos, clm.residuals)) + geom_jitter() + facet_wrap(~actor)
...
first.scenario.df <- propfull.df %>% filter(stage == 1) %>% select(Entidad, year, SCORE) %>% rename(FS_SCORE = SCORE)
second.scenario.df <- propfull.df %>% filter(stage == 3) %>% select(Entidad, year, SCORE) %>% rename(SS_SCORE = SCORE)
winningscores.df <- propfull.df %>% filter(stage == 5) %>% select(Entidad, year, SCORE) %>% rename(Win_SCORE = SCORE)

##Rejoin winning scores to the matching dataframe
win_match.df <- matching.df.1 %>% inner_join(first.scenario.df, by=c("Entidad", "year"))
win_match.df %<>% inner_join(second.scenario.df, by=c("Entidad", "year"))
win_match.df %<>% inner_join(winningscores.df, by=c("Entidad", "year"))
...
##########################################################################################
partisan_FS_SCORE.df <- matching.df.1 %>% filter(stage == 2) %>% select(Entidad, year, SCORE, actor) %>% rename(partisan_FS_SCORE = SCORE)
partisan_SS_SCORE.df <- matching.df.1 %>% filter(stage == 4) %>% select(Entidad, year, SCORE, actor) %>% rename(partisan_SS_SCORE = SCORE)

win_match.df %<>% inner_join(partisan_FS_SCORE.df, by=c("Entidad", "year", "actor"))
win_match.df %<>% inner_join(partisan_SS_SCORE.df, by=c("Entidad", "year", "actor"))
