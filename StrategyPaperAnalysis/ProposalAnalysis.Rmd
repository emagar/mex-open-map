...
matching.df.coal %<>% filter(stage == 2 | stage == 4) %>% 
  filter(PROPOSED == TRUE) %>% group_by(edon, stage, year, govlevel) %>% 
...
## Subset out scores winning scores for each stage (1, 2, and final) as well as pull out each partisan score (stage 1 and 2)
first.scenario.df <- propfull.df %>% 
  filter(stage == 1) %>% 
  select(edon, year, SCORE) %>% 
  rename(FS_SCORE = SCORE)

second.scenario.df <- propfull.df %>% 
  filter(stage == 3) %>% 
  select(edon, year, SCORE) %>% 
  rename(SS_SCORE = SCORE)

winningscores.df <- propfull.df %>% 
  filter(stage == 5) %>% 
  select(edon, year, SCORE) %>% 
  rename(Win_SCORE = SCORE) %>% 
  na.omit

partisan_FS_SCORE.df <- propfull.df %>%
  filter(stage == 2) %>% 
  select(edon, year, SCORE, actor, govlevel) %>%
  rename(partisan_FS_SCORE = SCORE) %>%
  na.omit

partisan_SS_SCORE.df <- propfull.df %>%
  filter(stage == 4) %>% 
  select(edon, year, SCORE, actor, govlevel) %>% 
...
win_match.coal.df.tmp1 <- left_join(matching.df.coal, partisan_FS_SCORE.df, by=c("edon", "year", "actor", "govlevel"))
win_match.coal.df.tmp2 <- left_join(win_match.coal.df.tmp1, first.scenario.df, by=c("edon", "year"))
win_match.coal.df.tmp3 <- left_join(win_match.coal.df.tmp2, partisan_SS_SCORE.df, by=c("edon", "year", "actor", "govlevel"))
win_match.coal.df.tmp4 <- left_join(win_match.coal.df.tmp3, second.scenario.df, by=c("edon", "year"))
win_match.coal.df.tmp5 <- left_join(win_match.coal.df.tmp4, winningscores.df, by=c("edon", "year"))

WM_coal.df <- win_match.coal.df.tmp5
##Create dummy matching minimum scores to the winning score
#WM_coal.df %<>% group_by(edon, stage, year, govlevel) %>% 
  #mutate(ACTOR_WIN = if_else(SCORE == Win_SCORE, 1, 0)) %>%
  #mutate(ALG_WIN = if_else(FS_SCORE == Win_SCORE, 1, 0))
```
