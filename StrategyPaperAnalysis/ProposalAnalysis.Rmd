...
source("Normalize.R",encoding='native.enc')
# integrates in plans from MxDistritos
source("integrateMxdistritos.R",encoding='UTF-8')

#TODO: Entidad character encodings create brittleness here. Normalize.R should be refactored to add an edon code, and to use that for merging.
```
...
```{r}
ys.proposed <- propfull.df %>% group_by(actor,Entidad,year) %>% summarise(numproposals=sum(PROPOSED,na.rm=TRUE))
ys.control<- grule.df %>%  controlByWindow(2000,2013) %>% transmute(Entidad=state, year=2013,                                 singlecontrolparty=if_else(singlecontrol,primary,"NONE"))

ys.control %<>% bind_rows (grule.df %>%  controlByWindow(2000,2017) %>% transmute(Entidad=state, year=2017,                              singlecontrolparty=if_else(singlecontrol,primary,"NONE")))
...
#NOTE: All of the code below needs to be rewritten to use tidy analysis and manipulation correctly. It produces results that are inconsistent with the previoius code, and one version must be in error. 
...
# NOTE: Year dummy is not necessary as year is already a 2-level variable. Why are we adding this? 
