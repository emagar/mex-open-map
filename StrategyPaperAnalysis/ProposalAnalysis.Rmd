...
ys.proposed %<>% left_join(alternation.df, by = c("edon", "year"))
...
prop.H1.df <- prop.H1.df%>%mutate(didControl=ifelse(actor==primary,1,0)) %>% mutate(havewon=ifelse(actor==(partyseq[["values"]])))

prop.H1.df <- prop.H1.df %>% group_by(edon, actor, year) %>%mutate(StratPos.1 = ifelse(didControl & singlecontrol, 1, 0)) %>%
  mutate(StratPos.2 = ifelse(didControl & !singlecontrol, 1, 0)) %>%
  
  mutate(StratPos.3 = ifelse(!didControl & percentsingle < 0.5 & numparty=3,1, 0)) %>%
  mutate(StratPos.4 = ifelse(!didControl & percentsingle < 0.5 & numparty=4 3, 1, 0))
