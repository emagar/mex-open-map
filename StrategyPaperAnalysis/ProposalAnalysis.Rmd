...
matching.df.coal %<>% filter(stage == 2 | stage == 4) %>% filter(PROPOSED == TRUE) %>% group_by(edon, stage, year, govlevel) %>% mutate(inCoalition=ifelse(CoalitionStratPos > 0, 1, 0)) #may need to also filter out parties not in coalitions for simplicity, will check the dataframe
...
win_match.coal.df <- matching.df.coal %>% inner_join(first.scenario.df, by=c("edon", "year"))
win_match.coal.df %<>% inner_join(second.scenario.df, by=c("edon", "year"))
win_match.coal.df %<>% inner_join(winningscores.df, by=c("edon", "year"))

##Create dummy matching minimum scores to the winning score
win_match.coal.df %<>% group_by(edon, stage, year, govlevel) %>% mutate(WIN = ifelse(SCORE == Win_SCORE, 1, 0))
...
major.score <- matching.df.coal %>% filter(actortype == "major" & stage == 2) %>% select(edon, year, SCORE, actor) %>% rename(Maj_Party_Score = SCORE)
matching.df.coal <- left_join(matching.df.coal, major.score, by=c("edon", "year"))
matching.df.coal <- matching.df.coal %>%
  group_by(edon, year, coalition) %>%
  mutate(maj_min_match = if_else(SCORE == Maj_Party_Score, 1, 0))

matching.df.coal %>% filter(actortype == "minor") %>%
  group_by(edon, govlevel.x, year, coalition, maj_min_match) %>%
  filter(year == 2013) %>%
  summarise(., n()) %>%
  gt()
...
Vraw.2018 %<>% pivot_longer(., cols = c("pan", "pri", "prd", "pvem", "pt", "mc", "pna", "morena", "pes", "panc", "pric", "morenac"),
                            names_to = "actor",
                            values_to = "votes.2018")
...
Vraw.2018 %<>% na.omit() %>%
  group_by(edon, disn, actor) %>%
  summarise(., votes.2018 = sum(votes.2018),
            lisnom.2018 = sum(lisnom),
            efec.2018 = sum(efec),
            dpanc.2018 = sum(dpanc),
            dpric.2018 = sum(dpric),
            dmorenac.2018 = sum(dmorenac)) %>%
    mutate(VoteShare.2018 = (votes.2018/efec.2018)*100)

#Defines the "rentability" of each each district by party vote share in said district.
#A measure of a party's electoral prospects in a district (Extremely Rentable == GOOD electoral prospects)
Vraw.2018 %<>% rowwise() %>% mutate(
    Rentability = case_when(
      VoteShare.2018 > 90    ~ 1, # Extremely Rentable
      VoteShare.2018 > 50    ~ 2, # Rentable
      VoteShare.2018 > 30    ~ 3, # Moderately Rentable
      VoteShare.2018 <= 30   ~ 4, # Weakly Rentable
    )
  )
...
Vraw.2018 %>% 
  ...
  summarise(., n()) %>%
  gt() %>%
  cols_label("actor" = "Party", "Rentability" = "District Scores")
...
Vraw.2009 %<>% na.omit() %>%
  group_by(edon, disn, actor) %>%
  summarise(., votes.2009 = sum(votes.2009),
            lisnom.2009 = sum(lisnom),
            efec.2009 = sum(efec)) %>%
    mutate(VoteShare.2009 = (votes.2009/efec.2009)*100)

#Defines the "rentability" of each each district by party vote share in said district.
#A measure of a party's electoral prospects in a district (Extremely Rentable == GOOD electoral prospects)
Vraw.2009 %<>% rowwise() %>% mutate(
    Rentability = case_when(
      VoteShare.2009 > 90    ~ 1, # Extremely Rentable
      VoteShare.2009 > 50    ~ 2, # Rentable
      VoteShare.2009 > 30    ~ 3, # Moderately Rentable
      VoteShare.2009 <= 30   ~ 4, # Weakly Rentable
    )
  )
...
#Create a column for actor (to more easily merge with other datasets), votes are values associated w/ each actor category
Vraw.2006 %<>% pivot_longer(., cols = c("pan", "pric", "prdc", "pna", "asdc"),
                            names_to = "actor",
                            values_to = "votes.2006")
Vraw.2006 %<>% na.omit() %>%
  group_by(edon, disn, actor) %>%
  summarise(., votes.2006 = sum(votes.2006),
            efec.2006 = sum(efec)) %>%
    mutate(VoteShare.2006 = (votes.2006/efec.2006)*100)

#Defines the "rentability" of each each district by party vote share in said district.
#A measure of a party's electoral prospects in a district (Extremely Rentable == GOOD electoral prospects)
Vraw.2006 %<>% rowwise() %>% mutate(
    Rentability = case_when(
      VoteShare.2006 > 90    ~ 1, # Extremely Rentable
      VoteShare.2006 > 50    ~ 2, # Rentable
      VoteShare.2006 > 30    ~ 3, # Moderately Rentable
      VoteShare.2006 <= 30   ~ 4, # Weakly Rentable
    )
  )
...
```{r}
#Create a column for actor (to more easily merge with other datasets), votes are values associated w/ each actor category
Vraw.2003 %<>% pivot_longer(., cols = c("pan", "pri", "prd", "pvem", "pt", "conve", "psn", "pas", "mp", "plm", "fc", "pric"),
                            names_to = "actor",
                            values_to = "votes.2003")
Vraw.2003 %<>% na.omit() %>%
  group_by(edon, disn, actor) %>%
  summarise(., votes.2003 = sum(votes.2003),
            efec.2003 = sum(efec)) %>%
    mutate(VoteShare.2003 = (votes.2003/efec.2003)*100)

#Defines the "rentability" of each each district by party vote share in said district.
#A measure of a party's electoral prospects in a district (Extremely Rentable == GOOD electoral prospects)
Vraw.2003 %<>% rowwise() %>% mutate(
    Rentability = case_when(
      VoteShare.2003 > 90    ~ 1, # Extremely Rentable
      VoteShare.2003 > 50    ~ 2, # Rentable
      VoteShare.2003 > 30    ~ 3, # Moderately Rentable
      VoteShare.2003 <= 30   ~ 4, # Weakly Rentable
    )
  )
...
#Create a column for actor (to more easily merge with other datasets), votes are values associated w/ each actor category
Vraw.2000 %<>% pivot_longer(., cols = c("panc", "pri", "prdc", "pcd", "parm", "dsppn"),
                            names_to = "actor",
                            values_to = "votes.2000")

Vraw.2000 %<>% na.omit() %>%
  group_by(edon, disn, actor) %>%
  summarise(., votes.2000 = sum(votes.2000),
            efec.2000 = sum(efec)) %>%
    mutate(VoteShare.2000 = (votes.2000/efec.2000)*100)

#Defines the "rentability" of each each district by party vote share in said district.
#A measure of a party's electoral prospects in a district (Extremely Rentable == GOOD electoral prospects)
Vraw.2000 %<>% rowwise() %>% mutate(
    Rentability = case_when(
      VoteShare.2000 > 90    ~ 1, # Extremely Rentable
      VoteShare.2000 > 50    ~ 2, # Rentable
      VoteShare.2000 > 30    ~ 3, # Moderately Rentable
      VoteShare.2000 <= 30   ~ 4, # Weakly Rentable
    )
  )
...
Registered Voters + Vote Counts (County/State/District)

Vote Share data Aggregation
```{r}
Country1 <- Vraw.2000 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2000))
Country2 <- Vraw.2003 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2003))
Country3 <- Vraw.2006 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2006))
Country4 <- Vraw.2009 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2009))
Country5 <- Vraw.2012 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2012))
Country6 <- Vraw.2015 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2015))
Country7 <- Vraw.2018 %>% 
  as_tibble() %>%
  summarise(., efec = sum(efec.2018))

Country_Votes.df <- rbind(Country1, Country2, Country3, Country4, Country5, Country6, Country7)
Country_Votes.df %<>% mutate(id = row_number())
Country_Votes.df %>% ggplot(aes(x=id, y = efec)) + geom_line() + xlab("")
```

Merging Vhat values + Prediction function
```{r}
...
masterfile_1 <- left_join(x = Vhat.2015, y = Vraw.2015, by = c("edon", "seccion"))
