...
Admin - CLV, Algorithm, Junta, INE, ife, escenario3_derfe
Other - PRD51, inegi
```{r}
#Categorize types of parties
#break out into a separate data frame and save for future use

propfull.df$actor <- gsub("PANAL", "PNA", propfull.df$actor) #Comes from ENA merge, "Eric's code," need to fix that at the source. In the mapping file too - update mapping file (micah). 
...
one.df <- propfull.df %>% filter(propfull.df$actor == "Junta")
...
table(prop.H1.df$govlevel)
check.df <- prop.H1.df %>% filter(govlevel == is.na(govlevel))
table(check.df$edon)
#Check the categories
#Exhaustive and mutually exclusive, like we want.
```
Cleaning up the data
Removing obs w/ NA's in Proposed
Figuring out why there are NA's in govlevel
```{r}
prop.H1.df <- prop.H1.df %>% drop_na(PROPOSED)
```

Some visualizations
```{r}
prop.H1.df %>% ggplot(aes(StratPos)) + geom_histogram() +facet_wrap(~year)
prop.H1.df %>% ggplot(aes(StratPos)) + geom_histogram() + facet_wrap(~govlevel)
#Now break it down by year
prop.H1.df %>% ggplot(aes(actor)) + geom_bar() + facet_wrap(~year)
```

Models
```{r}
#1. Logistic
Mod.1 <- prop.H1.df %>% glm(PROPOSED ~ as.factor(StratPos), data=., family = binomial)
Mod.2 <- prop.H1.df %>% glm(PROPOSED ~  as.factor(StratPos)*as.factor(year), data=., family = binomial)
Mod.3 <- prop.H1.df %>% glm(PROPOSED ~ as.factor(StratPos) +  as.factor(govlevel)*as.factor(year), data=., family = binomial)
...
length(prop.H1.df$PROPOSED)
table(prop.H1.df$PROPOSED)

t <- table(Mod.1, prop.H1.df$PROPOSED)
t
```
