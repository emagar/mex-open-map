...
prop.H1.df <- prop.H1.df %>%
  ...
  mutate(StratPos.3 = ifelse(!CONTROL & (primary==actor), 1, 0)) %>%
  mutate(StratPos.4 = ifelse(!CONTROL & !(primary==actor) & ((secondary==actor) | (tertiary==actor)) & (numparty <= 2), 1, 0)) %>%
  mutate(StratPos.5 = ifelse(!CONTROL & !(primary==actor) & ((secondary==actor) | (tertiary==actor)) & (numparty >=3), 1, 0)) %>%
  mutate(StratPos.6 = ifelse(!CONTROL & !(primary==actor) & !(secondary==actor) & !(tertiary==actor) & (numparty <= 2), 1, 0)) %>%
  mutate(StratPos.7 = ifelse(!CONTROL & !(primary==actor) & !(secondary==actor) & !(tertiary==actor) & (numparty >= 3), 1, 0))
