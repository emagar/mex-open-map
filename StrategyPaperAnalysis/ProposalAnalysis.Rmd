...
```{r}
library(questionr)
#Why "control" for year in the model prior?
ys.proposed.13 <- ys.proposed%>%
  filter(year == 2013)
tabfit.13 <- ys.proposed.13 %>% xtabs(~didPropose+didControl,data=.)
print(tabfit.13)
#the data looks unbalanced from the table
glm.mod.13 <- glm(didPropose~didControl, data=ys.proposed.13, family = binomial())
summary(glm.mod.13)
ggcoef(glm.mod.13)

ys.proposed.17 <- ys.proposed%>%
  filter(year==2017)
tabfit.17 <- ys.proposed.17 %>% xtabs(~didPropose+didControl,data=.)
print(tabfit.17)
glm.mod.17 <- glm(didPropose~didControl, data=ys.proposed.17, family = binomial())
summary(glm.mod.17)
ggcoef(glm.mod.17)

#Trying the linear models.
lm.mod.b <- lm(numproposals~didControl, data=ys.proposed)
summary(lm.mod.b)

lm.mod.13.b <- lm(numproposals~didControl, data=ys.proposed.13)
summary(lm.mod.13.b)

lm.mod.17 <- lm(numproposals~didControl, data=ys.proposed.17)
summary(lm.mod.17.b)

#How about the number of proposals. Another take on 
