...
matching.df.1 %<>% filter(stage == 2 | stage == 4) %>% filter(INVALID == FALSE) %>% group_by(edon, stage, year, govlevel) %>% mutate(minSCORE = ifelse(SCORE == min(SCORE), 1, 0)) %>% mutate(minSCOREnum = min(SCORE))
