...
glm.mod <- ys.proposed %>% glm(didPropose ~ as.ordered(StratPos), family = binomial(link = "logit"), data=.)
summary(glm.mod)
...
model.results <- ys.proposed %>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+as.factor(year)+(1|actor),., Hess = TRUE)
