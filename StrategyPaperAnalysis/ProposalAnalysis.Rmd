...
model.results <- ys.proposed %>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+as.factor(year)+(1|actor),.)
...
#clmm2
clmm2.mod <- clmm2(location = as.ordered(didPropose)~ as.ordered(StratPos) + as.factor(year), random = as.factor(actor), data = ys.proposed, Hess = TRUE)
summary(clmm2.mod)
