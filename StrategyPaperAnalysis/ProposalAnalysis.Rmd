...
coalitions.df %<>% rowwise() %>% mutate(
    CoalitionType = case_when(
      (coalition == "PAN-PNA" | coalition == "" | coalition == "") ~ "PAN", 
      ...
      coalition == "PRD-MC-PT" | coaltion == "PRD-PT" | coalition == "PRD-PT-MC" | coalition == "MORENA-PT-ES" | coaltion == "PRD-PNA" | coalition == "PRD-PT-ES" |      coaltion == "PRD-PT-PNA" ~ "Left"
    )
  )
...
win_match.coal.df %>% filter((actortype == "major" | actortype == "minor") & stage.x == 2) %>%
  group_by(year, coalition) %>% summarise(WIN = sum(Scen.2.1.A), denominator = sum(n()), percent = (WIN/denominator)*100) %>%
  gt() %>%
    cols_label("coalition" = "Party", "WIN" = "Accepted Proposals", "denominator" = "Total Proposals", "percent" = "% Accepted") %>%
...
win_match.coal.df %>% filter((actortype == "major" | actortype == "minor") & stage.x == 4) %>%
  group_by(year, coalition) %>% summarise(WIN = sum(Scen.2.1.A.1), denominator = n(), percent = (WIN/denominator)*100) %>%
  gt() %>%
    cols_label("coalition" = "Party", "WIN" = "Accepted Proposals", "denominator" = "Total Proposals", "percent" = "% Accepted") %>%
