...
#CLM model first
<<<<<<< HEAD
clm.mod <- ys.proposed %>% filter(partytype=="minor" || partytype=="major") %>% clm(as.ordered(didPropose)~as.ordered(StratPos), data=.)
=======
clm.mod <- ys.proposed %>% clm(as.ordered(didPropose)~as.ordered(StratPos), data=.)
>>>>>>> a1d9793ac355fba93ce6c6d1d5061a977bd648b6
summary(clm.mod)

#example of ordinal model only, seems to have numerical problems
model.results <- ys.proposed %>% clmm(as.ordered(didPropose)~as.ordered(StratPos)+(1|actor),.)
...
```{r}
matching.df.1 <- propfull.df %>% filter(year == 2013 & govlevel == "CNV" & Entidad == "Baja California" & SCORE > 0) %>% mutate(compSCORE = case_when(actor == "Algorithm" ~ SCORE))
