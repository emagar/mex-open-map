...
res_outcomes_unrolled.df %>% 
  ...
  group_by(year,creators) %>% relocate(type) %>% select(-denominator) %>%
    ...
    gt_theme_538() %>%
  gtsave(., "tab1.png", path="C:/Users/Brandon/Documents/mex-open-map/StrategyPaperAnalysis/")
...
VS_R8_master.df <- rbind(VS_R8_chart1, VS_R8_chart2, VS_R8_chart3, VS_R8_chart4)
...
VS_R8_master.df %>%
  group_by(year, actor, violation) %>%
  summarise(., mean(VS15), mean(VS18)) %>%
  gt()
