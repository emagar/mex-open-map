...
```{r}
# Table 4.1 for Row 1
proptrans.df %>% filter(actortype=="minor" | actortype=="major") %>% group_by(year) %>% summarise(proposed=sum(PROPOSED,na.rm=TRUE),invalid=sum(!VALIDPLAN),rejectionrate=invalid/proposed)

# Table 4.2 Col 1:2
proptrans.df %>% filter(actortype=="minor" | actortype=="major") %>% group_by(year,actor) %>% summarise(proposed=sum(PROPOSED,na.rm=TRUE),invalid=sum(!VALIDPLAN),rejectionrate=invalid/proposed)
