...
zoom2<-planProp.df %>% filter(VALIDPLAN) %>% group_by(planid) %>% unnest(cols=stages) %>% filter(year==2017, edon==20) %>% 
...
res_outcomes_unrolled.df %<>% bind_rows(
  ...
  group_by(year,creators) %>% 
  summarise(  events = n(), type = "Win inviolation of the rules", denominator=unique(denominator) )
)
