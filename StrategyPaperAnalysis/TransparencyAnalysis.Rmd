...
results<-propfull.df%>% xtabs(PROPOSED~actor+govlevel+year+stage,data=.)
ftable(results)
```
## Invalidations (Tables 4.1 R1, 4.2 C1:2)
```{r}
# Table 4.1 for Row 1
propfull.df %>% filter(actortype=="minor" | actortype=="major") %>% group_by(year) %>% summarise(proposed=sum(PROPOSED,na.rm=TRUE),invalid=sum(INVALID,na.rm=TRUE),rejectionrate=invalid/proposed)
# Table 4.2 Col 1:2
propfull.df %>% filter(actortype=="minor" | actortype=="major") %>% group_by(year,actor) %>% summarise(proposed=sum(PROPOSED,na.rm=TRUE),invalid=sum(INVALID,na.rm=TRUE),rejectionrate=invalid/proposed)
```
#Ambiguous Actors
*  All are in 2013
...
```{r}
ghostprop.df <-propfull.df %>% filter(actortype!="minor" & actortype!="major") %>% filter(stage==2 | stage==4) %>% filter(PROPOSED==TRUE)

# Table 4.1 R2
ghostprop.df %>% group_by(year,actor) %>% summarise(ghostproposals=sum(PROPOSED,na.rm=TRUE))
