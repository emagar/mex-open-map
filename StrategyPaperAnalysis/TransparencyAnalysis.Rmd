...
# TODO --> recode {INE, Junata, CLV} -> Admin; PRD51 to PRD
```
## INE Interventions
```{r}
...
#plots
pl1<-planProp.df %>% group_by(planid) %>% unnest(cols=stages) %>% filter(year==2013) %>% 
  ggplot(aes(x=stages,y=rscore,color=creator_label,group=planid)) +geom_point()+geom_line()+
  facet_grid(rows=vars(edon), cols=vars(year), scales="free_y")+theme(legend.position = "none")
pl2<-planProp.df %>% group_by(planid) %>% unnest(cols=stages) %>% filter(year==2017) %>% 
   ggplot(aes(x=stages,y=rscore,color=creator_label,group=planid))+geom_point()+geom_line()+
...
```{r}
#TODO - examine gtsummary() for tables
