...
```{r}
contestSum.df %<>% rowwise() %>% mutate(lowestvalid = min(phase1_start,phase2_plans,phase3_win,phase4_plans),phase2_lowest = min(phase1_start,phase2_plans),best_lost=final_win!=lowestvalid)
...
planProp.df %<>% left_join(contestSum.df %>% select(year,edon,lowestvalid,phase2_lowest))
