...
tmp2013s2.df <- normalizeEvent(subset(INE2013s2.df,select=-CNV.Ganador))
...
```{r}
results<-xtabs(formula=PROPOSED~actor+govlevel+scenario,data=proposals.df)
```
...
winscores <- subset(proposals.df,actor=="Tercer", select=c(SCORE,year,Entidad))
print (winscores)
```
